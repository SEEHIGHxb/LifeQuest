...
export function renderMethodology(containerId, state) {
  ...
  container.innerHTML = `
    ...
    <div class="card">
      ...
      <p class="aspect-blurb">${t("The Balance Index is this app's own summary figure, not a published or validated measure — unlike the eight aspect scores and their percentiles, no research proposes it and nothing outside this app uses it. It is the harmonic mean of your eight aspect scores, which means your lowest aspect pulls it down hardest: eight scores of 70 give an index of 70, while seven scores near 79 with one at 10 give 42, even though both average 70. That is deliberate — a single number that rewarded a high average would reward neglecting an aspect entirely, and this app is about balance.")}</p>
      <p class="aspect-blurb">${t("Because the index is dominated by your weakest aspect, raising a low score moves it far more than raising an already-high one. Treat it as a prompt about where attention is missing, not as a verdict on your life.")}</p>
